...
data transfers with task execution. Furthermore, the runtime API
...
There are only a few choices for such a runtime API, especially
if limiting oneself to alternatives that have support from
...
However, we believe that OpenCL is better for a portability
...
doesn't support all the CUDA capabilities, unlike the Coarse Grained
SVM available in OpenCL~\pj{Colleen (?) TODO: pls check}. It also
...
Heterogeneous System Architecture (HSA) is an open specification
that also defines a low level portability layer~\cite{HSA}. A key differentiating
feature of HSA was that it standardized on shared virtual memory,
...
Both LevelZero~\cite{LevelZero} and ROCr~\cite{ROCr} are lightweight hardware
abstraction layers that could be useful in case they had more vendors
supporting them actively. LevelZero has the benefit that it uses SPIR-V
as the device program representation, which made it easy to add support
to \hipstar for interfacing to LevelZero directly to control Intel GPU
devices. ROCr, however, is only targeted to AMD GPUs and cannot ingest
SPIR-V at the time of this writing~\pj{check this}.
