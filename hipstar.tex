...
\title{hipstar: Compiling HIP/CUDA Programs to an Open Cross-Vendor Standard Software Stack}

%\author{pekka.jaaskelainen }
%\date{March 2023}

\author{Pekka Jääskeläinen, Henry Linjamäki, Michal Babej, Peng Tu, Sarkar Sarbojit, Brice Videau, Colleen Bertoni, Kevin Harms, Paulius Velesko, Philip C. Roth, Rahulkumar Gaytri, Jisheng Zhao
        % <-this % stops a space
...
\pj{TODO: describe how we utilize USM to optimize the memory management.}
...
\subsection{Kernel Built-ins}

The \textit{hipstar} kernel library  implements the HIP math API, by using the OpenCL C math builtins, where possible.\pj{Something for Paulius to update after the rework?}
A lot of the functions in the math API have an equivalent OpenCL builtin with adequate accuracy guarantees. However, there are a few notable functions which don't map directly, and thus require software based emulation:
...
\pj{TODO: Also the basic math seems to differ in accuracy with CUDA offering higher accuracy in some cases (https://github.com/CHIP-SPV/chip-spv/issues/222). Discuss the challenges it poses here.}
\mb{TODO there is also a lot of math "intrinsics" - which i think map directly to GPU instructions in CUDA - and most of these are not implemented (a few have a native\_XYZ equivalent in OpenCL)}\pj{See above discussion. They cannot be used as default because their accuracy is not defined at all.}
...
cl\_intel\_required\_subgroup\_size & When calling warp-level primitives that depend on the 32/64 warp size or the thread id ordering. & Promotion to a generalized 'khr' extension being discussed. \\ 
    \hline
cl\_intel\_unified\_shared\_memory & When available, is used for optimized memory management.  & Public. \\
   \hline
...
Thanks to using the open standard OpenCL as the portability layer, various debugging and profiling tooling options are available to use with little to no additional effort. The following discusses a set of tools that were successfully adopted and used through $hipstar$.
...
\pj{This could be an interesting curiosity as soon as we (finally!) get PoCL-R open sourced.}
...
In order to assess the  compatibility level of \textit{hipstar}, we ported a set of well-known applications and libraries used widely in HPC domain that are commonly used with CUDA or HIP applications. The applications and which features or libraries of \hipstar they use are shown in Table~\ref{tab:applications}.
...
Here we focus on the Hartee-Fock (HF) algorithm used by a CUDA library in GAMESS (described extensively in Ref. \cite{gamess_cuda1}) which has been ported to HIP \cb{TODO find some reference for the porting effort}. The HF method is a common quantum chemistry method which is often the starting point for other higher-accuracy methods.  The HF method determines the molecular energy of a system by solving a set of non-linear eigenvalue equations iteratively.  It primarily involves the computation of $N^4$ two electron integrals (where $N$ is a measure of molecular system size) as well as matrix contractions of the two electron integrals once they are formed. 
...
\begin{figure}
     ...
     \begin{subfigure}[b]{0.5\textwidth}
        ...
        \includegraphics[width=1.0\textwidth]{figs/LZ-BE-comparison-Intel-dGPU.pdf}
         ...
         \label{fig:lz-comparison-igpu}
     \end{subfigure}
     \caption{\hipstar direct Level Zero backend execution time (normalized to 100) in comparison to the \hipstar OpenCL backend on top of Intel OpenCL and the layered PoCL-Level0 driver with two different GPUs.}
     \label{fig:lz-comparison}
\end{figure}

\pj{TODO: Straight to CUDA vs. CUDA to HIP to hipstar to OpenCL + to OpenCL to PoCL-CUDA to CUDA}
