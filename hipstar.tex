...
\pj{TODO: Discuss the shadow buffer implementation and its pitfalls. hipRegisterHostMemory() difficulties.}
\pj{TODO: Optimize the memory management by using USM.}

\pj{TODO: About the difficulties of SVM registration (kernel exec info). Need to register allocations "just in case" in case not sure they are not accessed. This leads to a race condition when another thread frees an allocation before the kernel is started or when it starts. Done easier in level0 where one can just set a flag.}
...
cl\_intel\_required\_subgroup\_size & When calling warp-level primitives that depend on the 32/64 warp size or the thread id ordering. & Promotion to a generalized 'khr' extension being discussed. \\ 
    \hline
cl\_intel\_unified\_shared\_memory & When available, used for optimizing shared memory usage. \pj{TBD} & Public. \\
   \hline
...
in the Google LLC vs. Oracle America, Inc. case that copying Java API for use in the Android OS was considered ``fair use'' in case it's done for compatibility purposes~\cite{JavaSupreme}:
...
However, since we, the \hipstar developers are engineers, not lawyers, we wanted to be extra careful that copyrights were not disrespected in any jurisdiction when adding support for the direct CUDA API calls by using an implementation approach where only the programmer's manual was consulted for the API reference when writing the CUDA compatibility headers to the \hipstar code base. 
...
\pj{TODO: Consider "obese" binaries: Allow adding target-specific binaries (but via the OpenCL binary interface) to the binary for faster initial launch.}
