...
In order to enable easy automated transition path from CUDA applications, HIP is largely a copy of a CUDA C/C++ API subset with a few minor differences and renamed functions. HIP alleviates the CUDA portability problem, but doesn't solve it satisfactorily due to AMD targeting primarily their self-specified low level ROCm APIs which are not actively supported on non-AMD platforms.
...
portability challenge. In contrast to previous solutions either require source-to-source
conversion from CUDA programs~\cite{SYLComatic}, that can lead to costly multiple codebase maintainance, or target binary-level compatibility of existing CUDA/HIP programs~\cite{ZLUDA} that
rely on reverse engineering proprietary APIs, \chipstar chooses a middle-ground approach which
enables source-modification-free compilation of HIP/CUDA programs to a runtime portable ```fat binary'' that utilizes solely open standards and can execute on any platform supporting the
required standard features without recompilation.

We make the following contributions valuable to the heterogeneous computing and HPC communities:
...
\begin{enumerate}
  ...
  \item evaluate its performance in comparison to converting the CUDA applications first to a popular open-standard based CUDA alternative SYCL~\cite{SYCL},
  \item extend an open source OpenCL implementation to expand the portability on more device types, \pj{To check if we have enough content to talk about within this aspect.} and
  \item provide a review for contemporary API options for implementing the portability layer for heterogeneous programming models.
\end{enumerate}

The rest of the article is organized as follows: \pj{To update:} Section~\label{section:portabilityAPIs} discusses the rationale for choosing OpenCL~\cite{OpenCL} and its device-side program representation SPIR-V~\cite{SPIRV} as the core APIs to support runtime portability.
Section~\ref{section:runtime} discusses the technical challenges related to the software stack. Section~\ref{section:compilation} describes the most relevant parts of the Clang/LLVM~\cite{LLVM}-based compiler support.
% Section~\ref{section:libraries} identifies the most important libraries that must be supported for CUDA/ROCm compatibility, and how they are supported in the proposed software platform. 
%The aspects related to directly building CUDA programs instead of converting them first to HIP are discussed in Section~\ref{section:directCUDA}. Debugging and profiling support is outlined in Section~\ref{section:debuggingAndProfiling}. 
Then we describe how the software platform portability was tested (Section~\ref{section:platformPortability}), how the source code compatibility was validated with case studies (Section~\ref{section:applications}). Performance evaluations are presented in Section~\ref{section:performance}. Section~\ref{section:relatedWork} overviews the related work to the proposed software platform and its components. 
Finally, conclusions and future work plans are laid out in Section~\ref{section:conclusions}.
...
\pj{TODO: This section can and should be shrunk significantly.}
...
GPU offerings. However, the goal of \chipstar is to
...
cover as many of the essential features of CUDA/HIP as possible
in order to exploit the potential performance benefits. This includes,
...
multiple accelerator vendors or those that have a portable long maintained open source
implementation. An open standard based model that has increased
in popularity is SYCL~\cite{SYCL}, which is one of the key
programming APIs in the Intel's oneAPI software stack. SYCL
resembles CUDA in being a C++-based single-source API and
could be a potential option for the runtime layer.
...
\pj{This paragraph should be reviewed by an OpenMP expert:}
...
define a device-side program representation, making future-proof cross-vendor
portable fat binary generation difficult. It also can be considered
...
unneccessary for a portability layer API.
...
implementations. In hindsight this requirement was likely too ambitious
too early, as system-wide virtual memory support is only recently becoming
widely available in hardware and still usually requires explicit allocation or mapping
calls from the programmer. HSA could have been a valid choice for a lightweight
portability layer, but activity on it has seized with mainly AMD using only
...
would require further standard extensions. It might be that in the future
...
case it had more vendors than Intel supporting it officially. For \chipstar,
Level Zero has a benefit that it uses the open standard SPIR-V as the device program
representation, which made it relatively easy to add an option for using
Level Zero directly to control Intel GPU devices as an alternative
to OpenCL~\cite{HIPLZ}.

Interestingly, OpenCL was originally created to provide an open cross-vendor alternative to the proprietary CUDA GPGPU programming model. It hasn't received wide support from application developers since it has been considered too low level and unproductive with the driver support lagging behind the proprietary alternatives. However, as demonstrated by \chipstar, thanks to the official support from multiple vendors for the version 3.0 of the standard and multiple long-maintained open source implementations available, OpenCL/SPIR-V provide a great portability layer for implementing other higher-level programming models and APIs on top.

While the main program of a heterogeneous application is typically compiled to a native instruction-set binary of a selected host CPU, using an IR target and its just-in-time compilation for the device programs enables portability of the application binary across a diversity of co-processors.
A major design decision for \chipstar was to use SPIR-V for the device program representation.
%, a portable and standardized kernel program Intermediate Representation (IR) format that is supported by multiple OpenCL implementations and that also now has solid open source infrastructure available.
%
%Overall,
SPIR-V~\cite{SPIRV} seemed the best choice for the \chipstar’s ``fat binary'' format’s IR since it is supported both in later OpenCL standard as well as Vulkan. It also has good LLVM-based conversion tools available as open source. Therefore, SPIR-V support was set early on as a requirement for \chipstar supported OpenCL targets.  Unfortunately, its consumption support is not widespread in OpenCL implementations at the time of this writing, but hopefully this will change as the OpenCL implementations and input producers such as \chipstar mature. Notably, neither AMD's nor NVIDIA's OpenCL drivers support SPIR-V at the time of this writing, but the situation is changing with open source implementations RustiCL and PoCL~\cite{PoCL} having SPIR-V supported drivers, for example, PoCL-CPU being able to expand the portability to ARM CPUs.
...
\section{Implementing HIP/CUDA Runtime on OpenCL/SPIR-V}
...
because they do not allow pointer arithmetics or passing addresses in other data
...
accessible directly by the host, and is not swapped out to disk. The primary APIs to this
functionality are
\func{cudaHostAlloc()} and \func{cudaHostRegister()}. The former allocates pinned
memory directly and the latter pins a previous device allocation. \func{cudaHostAlloc()}
...
\pj{TODO Henry: Something about the textures implementation? It might need some explanation in the compiler part too?}
...
cl\_ext\_cuda\_math     & Implement math functions and intrinsics with precision requirements that match CUDA's. To enable more optimized intrinsics. & To be proposed.  \\
    \hline
cl\_ext\_device\_side\_abort        & Implement \func{\_\_trap()} on the low-level runtime side. The current implementation requires compilter transformations. & Public.  \\
    \hline
...
cl\_intel\_unified\_shared\_memory & Used for optimized \func{cudaMalloc()} when available.  & Public. To promote to a general 'khr' extension. \\
   \hline
cl\_khr\_command\_buffer            & For optimized implementation of CUDA graph re-execution. & Public. SVM command extension proposed.  \\
    \hline
...
\section{Application Porting Examples}
\label{section:applications}

In order to test \chipstar in practice with more complex real world applications, in addition to the benchmarks presented in the next section, we ported a set of relatively complex HIP/CUDA-based HPC applications and libraries using \chipstar. The test environment for the applications was the Aurora supercomputer utilizing Intel Datacenter Intel® Data Center GPU Max Series as the accelerator part.
...
The code base is primarily in Fortran 77/90 with some C/C++ and a CUDA library converted to HIP by a 3rd party. Recently a new GPU version of the Hartree-Fock (HF) and RI-MP2 methods were implemented in CUDA which scales to 4096 nodes on Summit, an Nvidia V100-based supercomputer \cite{gamess_cuda1, gamess_cuda2, summit}.
...
We evaluated the \chipstar performance by assessing the runtime of the GAMESS
software package, comparing the performance on various HIP-capable GPU platforms,
...
The GAMESS port was executed with the Intel Datacenter Intel® Data Center GPU Max Series
component of the Aurora installation. Its performance was compared against ... with
the results displayed in Table~\ref{TODO}.

\cb{TODO: I need to add Performance: A100, PVC, Gen9, MI100. I should also add the peak BW and peak flops in a table}

\pj{To analyze and discuss the results.}

\subsection{HeCbench HIP vs. SYCL Performance}
...
3) The benchmark application verified itself and had to validate correctly on both software platforms.
4) ... \pj{Henry: please add the rest of the criteria we used for picking the applications.}
...
shown in Fig.~\ref{fig:online-compilation}.

\begin{figure*}
    \centering
    \includesvg[width=\linewidth]{figs/hecbench-intel-arc-a750}
    \caption{HIP vs. SYCL speed on Intel ARC A750.}
    \label{fig:online-compilation}
\end{figure*}

Interestingly, although both the DPC++ and \chipstar use very similar tools and components in
their runtime (OpenCL) and compilation flow (Clang/LLVM and SPIR-V as the device intermediate
format), we measured significant variance in the execution performance to both directions.
We analyzed the cases with the most dramatic differences and identified various explanations:
Many of the benchmarks executed very short kernel commands, making the benchmark actually
mostly measure the host API call execution speed. For example, the ``overlay'' benchmark
could be sped up signficantly by switching off the profiling command queue feature that
was accidentally left on by default. In some cases the device built-ins were more optimized
in \chipstar than in DPC++, in some cases it was the opposite. For example, when
we compared the \chipstar and DPC++ LLVM IRs of the device code for the ``nlll'' benchmark,
we found that only \chipstar performed ``if-conversion'', converting some of the very small
branches to conditional moves, which provided significant benefits. The
rest of the differing cases are likely having unidentified implementation differences
between the SYCL and the HIP variations, e.g., related to specification-mandated
synchronization of the memory transfers.
...
\pj{TODO: Mention the libraries implementation burden.}
...
\begin{itemize}
    ...
    \item Can we add more porting case studies? CP2K? Is the pruned-down Pytorch-HIP sensible or too pruned down? Exabiome?
\end{itemize}}

\pj{todo:
\begin{itemize}
    \item Add a small table of the optional OpenCL 3.0 features which are required for certain features.
    \item I removed the Libraries section as there was too little technical content. cuBLAS is too brief and highlights the fact that we support only a small subset of the NVIDIA or AMD libraries.
    ...
    \item The article reads mostly as a technical report. For a more "scientific article" we should add a bit more complex technical content, e.g., about the Graph implementation using command buffers.
     \item Can we add more porting case studies? CP2K? Is the pruned-down Pytorch-HIP sensible or too pruned down? Exabiome?
     \item We should add more pictures. It's too much text. Anyone any idea of what and where?
\end{itemize}}
