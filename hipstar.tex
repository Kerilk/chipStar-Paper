...
However, since we, the \hipstar developers are engineers, not lawyers, we wanted to be extra careful that no copyrights were disrespected in any jurisdiction when adding support for the direct CUDA API calls by using an implementation approach where only the programmer's manual was consulted for the API reference when writing the CUDA compatibility headers to the \hipstar code base. 
