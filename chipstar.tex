...
Section~\ref{section:runtime} discusses the technical challenges related to the software stack. Section~\ref{section:compilation} describes the most relevant parts of the Clang/LLVM~\cite{LLVM}-based compiler support. Section~\ref{section:libraries} identifies the most important libraries that must be supported for CUDA/ROCm compatibility, and how they are supported in the proposed software platform. Section~\ref{section:expandingCompat} discusses how we expanded the platform support using open source tooling\pj{to check}. The aspects related to directly building CUDA programs instead of converting them first to HIP are discussed in Section~\ref{section:directCUDA}. Debugging and profiling support is outlined in Section~\ref{section:debuggingAndProfiling}. Then we describe how the software platform portability was tested (Section~\ref{section:platformPortability}), how the source code compatibility was validated with case studies (Section~\ref{section:applications}). Performance evaluations are presented in Section~\ref{section:performance}. Section~\ref{section:relatedWork} overviews the related work to the proposed software platform and its components. 
Finally, conclusions and future work plans are laid out in Section~\ref{section:conclusions}.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{OpenCL and SPIR-V for Heterogeneous Platform Portability}
...
%
%Overall,
SPIR-V~\cite{SPIRV} seemed the best choice for the \chipstar’s ``fat binary'' format’s IR since it is supported both in later OpenCL standard as well as Vulkan. It also has good LLVM-based conversion tools available as open source. Therefore, SPIR-V support was set early on as a requirement for \chipstar supported OpenCL targets.  Unfortunately, its consumption support is not widespread in OpenCL implementations at the time of this writing, but hopefully this will change as the OpenCL implementations and input producers such as \chipstar mature. Notably, neither AMD's nor NVIDIA's OpenCL drivers support SPIR-V at the time of this writing, but the situation is changing with open source implementations RustiCL and PoCL~\cite{PoCL} having SPIR-V supported implementations in progress.
...
This section discusses the compilation techniques used by \chipstar.
We first introduce the overall compilation flow and then focus on
key challenges that needed to be tackled. Finally we summarize our
findings on the key needs to extend the OpenCL/SPIR-V standards
to bridge key feature gaps between the specifications.
...
allocations, which removes the unneccessary synchronizations for the majority of
benchmarks seen so far. Otherwise, due to the CUDA's memory model, each launcher kernel
can potentially access any previously allocated buffer, inducing significant unneccessary
data synchronization overheads in the common case where the kernels only access buffers
set through their arguments.
...
cl\_khr\_subgroup\_shuffle          & Warp-level shuffle operations. & Public. \\
    \hline
    \end{tabular}
    \caption{OpenCL 3.0 standard extensions that \chipstar can use currently or in the future to implement CUDA/HIP features if the application uses them. Status describes the state of the extension at the time of this article's publication. }
