...
\pj{It would be good if this paragraph was reviewed by an OpenMP expert:}
...
feature of HSA is that it standardizes on shared virtual memory,
...
to the explicit event synchronization and recording APIs are required which are mapped
to OpenCL out-of-order queues by \chipstar, if available in the target.
...
\section{Compiler Aspects}
\label{section:compilation}

This section discusses the compilation techniques used by \chipstar. We first introduce the overall compilation flow and then focus on key challenges that needed to be tackled. Finally we summarize our findings on the key needs to extend the OpenCL/SPIR-V standards to bridge key feature gaps between the specifications.
...
The porting examples are described in the following subsections.
...
In this section we evaluate the execution performance of \chipstar  with specific focus on overheads caused by layering HIP/CUDA on top of other API layers such as OpenCL and Level Zero.
